...
class Detail extends Component {
  ...
  //初始化渲染后触发
  componentDidMount() {
    const {iconfont, actions} = this.props;
    actions.getMembers(iconfont);
    actions.getPrds(iconfont, 'all');
  }

  render() {
    const {iconfont, members, prds, actions, inviteMemberShow, deleteMemberShow} = this.props;
    const options = [
      { label: 'mrd', value: 'mrd' },
      { label: 'prd', value: 'prd' },
      { label: '开发', value: 'dev' },
    ...
    return (
      <div className="mod-detail">
        <h2 className="detail-title">{iconfont.name}</h2>
        <Tabs defaultActiveKey="member">
            ...
            </Row>
            {
              deleteMemberShow && deleteMemberShow.show ?
                <Modal title="删除团队成员"
                       ...
                       onOk={() => actions.deleteMember(iconfont, deleteMemberShow.member)}
                >
            ...
            }
            {
              inviteMemberShow ?
                <InviteMember
                  ...
                  type="add"
                  okCallback={member => actions.inviteMember(iconfont, member)}
          ...
          <TabPane tab="prd详情" key="prd">
            <CheckboxGroup options={options} defaultValue={[]} onChange={value => actions.getPrds(iconfont, value)} />
            <br/>
            {
...
export default connect(state=> {
  return {
    iconfont: state.iconfonts.find(item=>item.id == state.router.params.iconfontId) || pageConfig.iconfont.iconfont,
